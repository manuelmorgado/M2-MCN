...
def pos_dif(func, axis, flst, ax1, ax2):
    ...
    return (min_dif, min_lst, ave, fg_lst, ave, step_N) #returns the list of minima and the average of the difference
...
#function for define the step size of an array (difference between i and i+1 element)
def stepsize(datalst):
    for i in range(len(datalst)):
        step_s.append(datalst[i] - datalst[i+1]);
        meanSS = np.mean(step_s);
    return meanSS

#function to determine the relative distance between minima of datasets, then plot vs Nr of steps
def jumpcal(poslst, minlst):
    
    dj_jj = [];
    i=0;
    while i < len(poslst)-1:
        dataj = poslst[i];
        datajj = poslst[i+1];
        ...
        Pij = dataj[0];
        Pfj = dataj[len(dataj)-1];

        Pijj = datajj[0];
        Pfjj = datajj[len(datajj)-1];

        print(i, i+1)
        print(Pij,Pfj,Pijj,Pfjj)

        Sp = (Pij - minj) + (minjj - Pijj);
        Sm = (Pij - Pijj);
        dj_jj.append((Sp - Sm));

        i+=1;

    # print(type(np.asarray(dj_jj)), np.shape(dj_jj[:]),len(dj_jj))
    plt.figure()
    plt.plot(np.reshape(dj_jj, len(dj_jj)))
...
# varW = 'dVoltagePM'    #drive voltage variable to plot
# varR = 'dVelocityPM'    #velocity variable to plot
# varE = 'Step_Size'  #step-size of the PM
...
datX = csv_import(os.path.join(parent_dir,foldNames[0],'data1.csv'),varX)
datZ = csv_import(os.path.join(parent_dir,foldNames[0],'data1.csv'),varZ)
# datW = csv_import(os.path.join(parent_dir,foldNames[0],'data1.csv'),varW)
# datR = csv_import(os.path.join(parent_dir,foldNames[0],'data1.csv'),varR)
# datE = csv_import(os.path.join(parent_dir,foldNames[0],'data1.csv'),varE)
...
for kk in foldNames:
    ...
    piezo_posY.append(datZ);
    peakOD_lst.append(datX);
    # dVoltagePM.append(datW)
    # dVelocityPM.append(datR)
    # Step_size.append(datE)

#creating list with data for calibration (peakOD, YposPM, dVoltage, dVelocity, Stepsize)  
x_axis = np.asarray(peakOD_lst);
data_lst = np.asarray(piezo_posY)
# VoltagePM_lst = np.asarray(dVoltagePM);
# VelocityPM_lst = np.asarray(dVelocityPM);
# Stepsize_lst = np.asarray(Step_size);
...
# ax6 = fig.add_subplot(2, 2, 6)

#computing difference between functions in data_lst
min_lst = []; # minima list
min_dif, min_lst, average, fg_lst, ave, step_N = pos_dif(Gauss, x_axis, data_lst, ax1, ax2); #difference
...
#computing the difference absolute distances considering the the way forward and backwards
jumpcal(x_axis, min_lst)

#calibration on the step number
# ax5.plot(min_dif, step_Nlst)

#calibration on the drive voltage
# ax6.plot(min_dif, drive_Vlst)
